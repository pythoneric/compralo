...
// In-process sliding-window limiter. Single API instance only; replace with a
// Redis-backed limiter (we already run Redis in compose) before horizontal scaling.
@Injectable()
export class SignInRateLimitGuard implements CanActivate {
  ...
  canActivate(ctx: ExecutionContext): boolean {
    ...
    const emailRetry = email
      ? this.check(`em:${email}`, EMAIL_WINDOW, now)
      : 0;
